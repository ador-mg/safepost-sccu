...
function SCCU(channels, host, postbackBaseUrl){
                          ...
                          function(msg) { 
                            ...
                            try {
                              ...
                              var options = {
                                host: othis.host,
                                port: 80,
                                path: '/' + othis.postbackBaseUrl + '/' + channel,
                                method: 'POST',
                                headers: headers
                              };
...
SCCU.prototype.listenToChannel = function (channel, receivemessage) {
  ...
  var subscriptionData = { "id" : this.id + "_" + guid() + "_" + channel, "desc": this.desc + " for channel: " + channel };

  console.log("=========================================");
  console.log("subscription: ", subscriptionData.id);
  console.log("Channel: " + channel + " = " + SUB_URL);
  console.log("=========================================");

  var sdk = new AeonSDK(SUB_URL, subscriptionData);
  sdk.subscribe(receivemessage, function control(msg){

    console.log("Channel (ctrl): " + channel + ":", msg);

    if(msg.code == 201){
      console.log("Failed with ", subscriptionData);
    }
