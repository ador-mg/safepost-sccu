...
SCCU.prototype.start = function(){
  ...
  for (var i = 0; i < this.channels.length; i++) {
    var channel = othis.channels[i];
    othis.listenToChannel(channel, function(msg) { 
                                    othis.handleMessage(msg, channel);
                                  });
  };

  othis.selfCheckTimeout = setTimeout(othis.selfCheck.bind(othis), 30000);
}
...
SCCU.prototype.handleMessage = function(msg, channel){
  try {
    console.log("Received message from " + channel + ". Posting to CPSS...");
